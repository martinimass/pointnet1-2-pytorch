import os
import sys
from indoor3d_util_arch import DATA_PATH, collect_point_label

BASE_DIR = os.path.dirname(os.path.abspath(__file__))
ROOT_DIR = os.path.dirname(BASE_DIR)
sys.path.append(BASE_DIR)

anno_paths = [line.rstrip() for line in open(os.path.join(BASE_DIR, 'meta_arch/anno_paths.txt'))]
...
for anno_path in anno_paths:
    print(anno_path)
    try:
        elements = anno_path.replace("\\","/").split('/')
